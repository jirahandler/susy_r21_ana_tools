...
int hfplotspt()
{
    ...
    const char *outputFileName = "Beff_vs_jetpt_p_np_npinc_hf-tt.png";

    // Load histograms from the ROOT files
    TFile *file1 = TFile::Open(inputFile1);
    TFile *file2 = TFile::Open(inputFile2);

    TH1F *hist1_jetpt_all = static_cast<TH1F *>(file1->Get("jetpt_2_all"));
    TH1F *hist1_jetpt_tag = static_cast<TH1F *>(file1->Get("jetpt_2_tag"));

    TH1F *hist2_jetpt_all = static_cast<TH1F *>(file2->Get("jetpt_np_all"));
    TH1F *hist2_jetpt_tag = static_cast<TH1F *>(file2->Get("jetpt_np_tag"));

    TH1F *hist3_jetpt_all = static_cast<TH1F *>(file2->Get("jetpt_p_all"));
    TH1F *hist3_jetpt_tag = static_cast<TH1F *>(file2->Get("jetpt_p_tag"));

    TH1F *hist4_jetpt_all = static_cast<TH1F *>(file2->Get("jetpt_np_inc_all"));
    TH1F *hist4_jetpt_tag = static_cast<TH1F *>(file2->Get("jetpt_np_inc_tag"));

    try
    {
        if (hist1_jetpt_all->GetEntries() == 0 || hist2_jetpt_all->GetEntries() == 0)
        {
            throw std::runtime_error("One or both histograms are empty. Skipping efficiency calculation. Aborting... I'll be bacc.");
        }

        TH1D *ratio1 = static_cast<TH1D *>(hist1_jetpt_tag->Clone());
        ratio1->Divide(hist1_jetpt_tag, hist1_jetpt_all, 1.0, 1.0, "B");

        TH1D *ratio2 = static_cast<TH1D *>(hist2_jetpt_tag->Clone());
        ratio2->Divide(hist2_jetpt_tag, hist2_jetpt_all, 1.0, 1.0, "B");

        TH1D *ratio3 = static_cast<TH1D *>(hist3_jetpt_tag->Clone());
        ratio3->Divide(hist3_jetpt_tag, hist3_jetpt_all, 1.0, 1.0, "B");

        TH1D *ratio4 = static_cast<TH1D *>(hist4_jetpt_tag->Clone());
        ratio4->Divide(hist4_jetpt_tag, hist4_jetpt_all, 1.0, 1.0, "B");
