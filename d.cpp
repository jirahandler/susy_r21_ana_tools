...
void d::Loop()
{
  ...
  for (Long64_t jentry=0; jentry<nentries; jentry++) {
    ...
    for (int ijet = 0; ijet<njet; ++ijet) {
      // jet selection
      if ((*jet_pt)[ijet]<20e3 || fabs((*jet_eta)[ijet])>2.5) continue;
      if ((*jet_pt)[ijet]<60e3 && (*jet_JVT)[ijet]<0.2) continue;
      ...
      double w_dl1r = -99; if (a>0 && b>0) w_dl1r = log(a/b);
    ...
    for (int ijet = 0; ijet < njet; ++ijet)
    {
      ...
      // llp jet flavor
      auto llpjf = (*jet_truthLLPJetLabel)[ijet];
      if (llpjf == 4)
                   llpjf = 1;
      else if (llpjf == 5)
                   llpjf = 2;
