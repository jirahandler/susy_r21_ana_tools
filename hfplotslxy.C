...
int hfplotslxy()
{
    ...
    const char *outputFileName = "Beff_vs_lxy_p_np_npinc_hf-tt.png";
    ...
    TH1F *hist1_lxy_tag = static_cast<TH1F *>(file1->Get("lxy_tag"));

    TH1F *hist2_lxy_all = static_cast<TH1F *>(file2->Get("lxy_np_all"));
    TH1F *hist2_lxy_tag = static_cast<TH1F *>(file2->Get("lxy_np_tag"));

    TH1F *hist3_lxy_all = static_cast<TH1F *>(file2->Get("lxy_p_all"));
    TH1F *hist3_lxy_tag = static_cast<TH1F *>(file2->Get("lxy_p_tag"));

    TH1F *hist4_lxy_all = static_cast<TH1F *>(file2->Get("lxy_np_inc_all"));
    TH1F *hist4_lxy_tag = static_cast<TH1F *>(file2->Get("lxy_np_inc_tag"));

    try
    {
        if (hist1_lxy_all->GetEntries() == 0 || hist2_lxy_all->GetEntries() == 0)
        {
            throw std::runtime_error("One or both histograms are empty. Skipping efficiency calculation. Aborting... I'll be bacc.");
        }

        TH1D *ratio1 = static_cast<TH1D *>(hist1_lxy_tag->Clone());
        ratio1->Divide(hist1_lxy_tag, hist1_lxy_all, 1.0, 1.0, "B");

        TH1D *ratio2 = static_cast<TH1D *>(hist2_lxy_tag->Clone());
        ratio2->Divide(hist2_lxy_tag, hist2_lxy_all, 1.0, 1.0, "B");

        TH1D *ratio3 = static_cast<TH1D *>(hist3_lxy_tag->Clone());
        ratio3->Divide(hist3_lxy_tag, hist3_lxy_all, 1.0, 1.0, "B");

        TH1D *ratio4 = static_cast<TH1D *>(hist4_lxy_tag->Clone());
        ratio4->Divide(hist4_lxy_tag, hist4_lxy_all, 1.0, 1.0, "B");
